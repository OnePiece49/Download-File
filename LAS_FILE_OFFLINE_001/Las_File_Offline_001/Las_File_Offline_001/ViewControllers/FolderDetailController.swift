...
class FolderDetailController: BaseController {
	...
	private let bottomActionView: BottomActionView = {
		...
		view.translatesAutoresizingMaskIntoConstraints = false
		view.allowAction = [.share, .copy, .move, .zip, .delete]
		return view
	}()

	// MARK: - Lifecycle
	init(viewModel: FolderDetailViewModel) {
	...
	private func setupNavbar() {
		...
		navbarView = NavigationCustomView(centerTitle: viewModel.folder.name,
...
extension FolderDetailController {
	private func bindViewModel() {
		...
		viewModel.onZipFileComplete = { [weak self] zipFile in
			self?.loadingView?.dismiss()
			self?.loadingView = nil

			guard let self = self, let zipFile = zipFile else { return }
			self.viewModel.toggleMode()

			let vc = ZipFilesSuccessController(zipFile: zipFile)
...
extension FolderDetailController: UITableViewDataSource, UITableViewDelegate {
...
extension FolderDetailController: PreviewItemPresentable, QLPreviewControllerDataSource {
...
extension FolderDetailController: HeaderActionViewDelegate, BottomActionViewDelegate {
	...
	func didSelectAction(_ action: BottomActionView.ActionType) {
		switch action {
			...
			case .copy:
				presentFileActionSheet(type: .copy)

			case .move:
				presentFileActionSheet(type: .move)
	...
	private func presentFileActionSheet(type: FileActionSheetController.ActionType) {
		let fileVM = FileActionSheetViewModel(type: type, files: viewModel.selectedFiles, currentFolder: viewModel.folder)
	...
	private func zipFiles() {
		loadingView = LoadingAnimationView()
		loadingView?.setMessage("Zipping file. PLease wait for a while")
		loadingView?.show()
		viewModel.zipFiles()
	}
...
// MARK: - FileActionSheetControllerDelegate
extension FolderDetailController: FileActionSheetControllerDelegate {
