...
//                button.contentHorizontalAlignment = .fill
        ...
        let spaceLeft = (leftButtons.last?.frame.maxX ?? 0) + 22
        let spaceRight = (rightButtons.last?.frame.minX ?? 0) - 22
        
        let maxSpace = max(spaceLeft, spaceRight)
        
        NSLayoutConstraint.activate([
            titleLabel.leftAnchor.constraint(equalTo: leftAnchor, constant: maxSpace),
            titleLabel.rightAnchor.constraint(equalTo: rightAnchor, constant: -maxSpace),
